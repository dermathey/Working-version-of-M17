...
Правило переименования: латиница (транслитерация русских слов), нижний регистр, `-` вместо пробелов и `_`. Убраны `★`, скобки, `&`, `+`, `[112558]`, суффиксы `(1)`, апострофы. Ошибки, которые были в исходных именах (автор, опечатка), **сохранены в транслитерации** и вынесены в вопросы к клинике (`QUESTIONS.md`). Исходное имя прайса было записано в Unicode NFD («Й» = «И» + U+0306); новое имя — ASCII, то есть заведомо NFC.
...
| `Новая_косметтология_1_том_Марголина.md` | `textbooks/novaya-kosmettologiya-1-tom-margolina.md` | Марголина А.А., Эрнандес Е.И. «Новая косметология», т. I, 2005 (опечатка «косметтология» из исходного имени сохранена, см. вопросы) |
...
Служебные файлы: `README.md` (обзор и найденные проблемы), `FILEMAP.md` (этот файл), `KNOWN_ISSUES.md` (таблица проблем), `QUESTIONS.md` (вопросы к клинике).
